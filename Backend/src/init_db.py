...
class User(Base):
    ...
    park_history = relationship(
        "ParkHistory", back_populates="user", cascade="all, delete"
    )
...
class Car(Base):
    ...
    car_img = Column(String, nullable = False, default = 'images/default.jpg')
...
class Owns(Base):
    ...
    car_id = Column(
        Integer, ForeignKey("cars.car_id", ondelete="CASCADE"), primary_key=True
    )
