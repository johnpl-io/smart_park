...
const CreateAccountPage = () => {
    ...
    const createAccount = async () => {
        try {
            ...
            const userDetails = await fetch(`http://localhost:5000/check-username?username=${username}`)
            ...
            const userInfo = await fetch(`http://localhost:5000/find-user-by-email?email=${email}`);
            const user = await userInfo.json();


            localStorage.setItem("username", user.userName);
            localStorage.setItem("user_id", Number(user.user_id));
